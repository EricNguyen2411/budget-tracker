...
export function significantTokens(note: string): Set<string> {
  // Hyphens and apostrophes are treated as part of a word, not a
  // separator — confirmed a real gap without this: "JB Hi-Fi" splits
  // into "hi" and "fi", both too short to survive the length-3 filter
  // below, leaving this brand with NO identifying tokens at all. A
  // second JB Hi-Fi purchase a day or two later (same-day matching
  // already works fine via date alone) could never be caught as a
  // possible duplicate through the cross-day path, since there was
  // nothing left to match on. Merging "Hi-Fi" into "hifi" first keeps
  // it as one token that clears the length filter.
  const tokens = note.toLowerCase().replace(/[-']/g, '').split(/[^a-z0-9]+/)
