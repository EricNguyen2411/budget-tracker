...
function goalInsights(accounts: Account[], transactions: Transaction[], referenceDate: Date): Insight[] {
  const results: Insight[] = []
  for (const account of accounts.filter((a) => isGoal(a))) {
    if (goalProgressFraction(account, transactions, referenceDate) >= 1) {
      results.push({
        icon: '✅',
        text: `${account.name} has reached its ${formatCurrency(account.goalTargetAmount ?? 0)} goal.`,
        sentiment: 'positive'
      })
      continue
    }
    const projected = projectedGoalCompletionDate(account, transactions, referenceDate)
    if (!projected) continue
    const projectedText = projected.toLocaleDateString('en-AU', { month: 'long', year: 'numeric' })

    if (account.goalTargetDate) {
      const target = new Date(account.goalTargetDate)
      const targetText = target.toLocaleDateString('en-AU', { month: 'long', year: 'numeric' })
      if (projected <= target) {
        results.push({ icon: '✅', text: `At your current pace, ${account.name} is on track for ${projectedText} — ahead of your ${targetText} target.`, sentiment: 'positive' })
      } else {
        results.push({ icon: '⚠️', text: `At your current pace, ${account.name} won\u2019t be ready until ${projectedText} — behind your ${targetText} target.`, sentiment: 'warning' })
      }
    } else {
      results.push({ icon: '📅', text: `At your current pace, ${account.name} will reach its goal around ${projectedText}.`, sentiment: 'neutral' })
    }
  }
  return results
}

export function generateInsights(categories: Category[], transactions: Transaction[], accounts: Account[] = [], referenceDate: Date = new Date()): Insight[] {
  return [
    ...goalInsights(accounts, transactions, referenceDate),
